...
def create_augmented_dataloader(args, dataset):
    ...
    original_dataset = dataset["train"].map(tokenize_function, batched=True)
    ...
    new_dataset = dataset["train"].shuffle(seed=42).select(range(5000))
    transformed_dataset = new_dataset.map(custom_transform, load_from_cache_file=False)

    transformed_dataset = transformed_dataset.map(tokenize_function, batched=True)
    ...
    augmented_dataset = datasets.concatenate_datasets([original_dataset, transformed_dataset])
